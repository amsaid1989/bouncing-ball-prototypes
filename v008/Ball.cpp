...
void calculate_ball_values_from_mass(Ball &b, float mass, float window_width) {
  ...
  b.initial_velocity = (1 / pow(b.mass, 0.35)) * 30000;
  if (b.initial_velocity > b.max_velocity) {
    b.initial_velocity = b.max_velocity;
  }
}
...
void change_ball_mass(Ball &b, float mass, float window_width) {
  b.mass_changed = true;
  calculate_ball_values_from_mass(b, mass, window_width);
}
...
bool move_ball(Ball &b, const Paddle &p, float window_width,
               float window_height, int &score, float delta) {
  bool increment_score = false;
